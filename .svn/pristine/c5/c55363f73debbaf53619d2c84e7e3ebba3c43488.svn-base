<template>
    <div class="container-section col-sm-11 col-md-12" id="containerSection">
        ...
        <div class="role-center col-md-5" v-bind:class="{'role-show':showRoleCenter}" id="roleCenter">
            <dl>
                <dt>角色名称</dt>
                <dd>
            ...
            <dl>
                <dt>角色状态</dt>
                <dd>
                    <input name="roleStatus" type="radio" value="KY" v-model="centerForm.stauts"/><label class="label-input">正常</label>
                    <!-- <input name="roleStatus" type="radio" value="frozen" v-model="centerForm.stauts"/><label class="label-input">冻结</label> -->
                    <input name="roleStatus" type="radio" value="JY" v-model="centerForm.stauts"/><label class="label-input">停用</label>
                </dd>
            </dl>
            <dl>
                <dt>备注</dt>
    ...
    export default{
        ...
        methods: {
            ...
            loadTreeData: function(resolve, addcomponent){
                Util.ajax(
                    '/newconsole/consoleapi/user/res',
                    {},
                    'get',
                    function(res){
                        if(res.code === 0){
            ...
            showRole: function(item){
                ...
                this.selectedNodes = [];
                Util.ajax(
                    '/newconsole/consoleapi/user/res/roleId',
                    {id:item.id},
                    'get',
                    function(res){
                        if(res.code === 0){
            ...
            deleteUser: function(id, index){
                ...
                mod_delSubmit = true;
                Util.ajax(
                    '/newconsole/consoleapi/user/role/delete',
                    {
                        "id": id
                    },
                    'post',
                    function(res){
                        mod_delSubmit = false;
                        if(res.code === 0){
                            // that.backupResult.splice(index,1);
                            // that.userlist = that.backupResult;
            ...
            roleSave: function(){
                ...
                Util.ajax(
                    '/newconsole/consoleapi/user/role/saveOrUpdate',
                    param_data,
                    'post',
                    function(res){
                        mod_saveSubmit = false;
...
<style>
    ...
    .userlist-li:hover .user-del,.userlist-li.active .user-del,.role-show{
        display: block;
    }
    .list-hide{
        display: none
